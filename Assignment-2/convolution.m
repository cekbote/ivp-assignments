...
% the filter is a square. If it is required that the image does not loose
% its dimentsions due to the convolution process it is required that the
% image should be padded.

function [img] = correlation(image, filter)
[f_row, f_col] = size(filter);
[i_row, i_col] = size(image);
img=zeros(i_row-f_row+1, i_col-f_col+1);
